package com.night.appinfo.util;
...
public class PageInterceptor implements Interceptor {
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        ...
        if (page != null) {
            ...
            // 获取分页Sql语句
            String pageSql = this.getPageSql(connection, page, sql);
    ...
    public void setProperties(Properties properties) {
        // mybatis配置文件中没有设置属性就不获取值
    }
    /**
     * 根据page对象获取对应的分页查询Sql语句，这里支持三种数据库类型，Mysql,Oracle,SQLServer,其它的数据库可自行扩展
     * @param page 分页对象
     * @param sql  原sql语句
     */
    private String getPageSql(Connection connection, Page page, String sql)
            throws SQLException {
        ...
        if (productName.contains("mysql")) {
            ...
            return this.getSQLServerPageSql(page, sqlBuffer);
        }
    ...
    private String getOraclePageSql(Page page, StringBuffer sqlBuffer) {
        ...
        sqlBuffer.insert(0, "select u.*, rownum rn from (")
                .append(") u where rownum < ")
                .append(offset + page.getPageCount());
        sqlBuffer.insert(0, "select * from (").append(") where rn >= ")
                .append(offset);

        // 上面的Sql语句拼接之后大概是这个样子：
        // select * from (select u.*, rownum rn from (select * from t_user) u
        // where rownum < 31) where r >= 16
        return sqlBuffer.toString();
    }
    /**
     * 获取SQLServer数据库的分页查询语句（只支持SQLServer2005以上版本）
     *
     * @param page      分页对象
     * @param sqlBuffer 包含原sql语句的StringBuffer对象
     * @return SQLServer数据库的分页查询语句
     */
    private String getSQLServerPageSql(Page page, StringBuffer sqlBuffer) {
        // 计算第一条记录的位置，SQLServer分页是通过row_number进行的，记录位置从0开始
        int offset = (page.getPageNo() - 1) * page.getPageCount();

        // SQLServer数据库的查询语句不支持order by排序
        sqlBuffer
                .insert(0,
                        "select * from (select row_number() over(order by (select 0))as rn,* from(");
        sqlBuffer.append(") tb) t ");
        sqlBuffer.append(" where rn>");
        sqlBuffer.append(offset);
        sqlBuffer.append(" and rn<=");
        sqlBuffer.append(offset + page.getPageCount());
        return sqlBuffer.toString();
    }
    /**
     * 给当前的参数对象page设置总记录数
    ...
    private void setTotalRecord(Object obj, MappedStatement mappedStatement,
                                Connection connection, Page page) {
